...
1. **Draft** (cron or on-demand) — gathers GitHub activity (merged PRs, releases, notable commits), generates an email draft via LLM, and creates a GitHub issue for human review. If your product has multiple [audiences](/configuration/product-config#audiences), each audience gets its own draft with a tailored voice.
2. **Send on Close** — when a draft issue is closed (approved), emails are sent to subscribers via [Resend](https://resend.com) or [Gmail](/configuration/environment#email-provider).
